...
# Read gene orthology table_____________________________________________________

#It may seem counter intuitive to read the lizard-mouse orthology table since I 
#will only use mouse data. But I will use the mouse column to arrange the count 
#tables to have the same rownames.
...
# Read Cao data 
## Count table__________________________________________________________________
Cao <- readRDS(paste0(path, "01_Data/CaoData/gene_count_cleaned.RDS")) %>%
...
### Rename genes________________________________________________________________
...
## Annotations__________________________________________________________________
#Read data
Cao_ann <- read.csv(paste0(path, "01_Data/CaoData/cell_annotate.csv")
...
# Read Soldatov data
## Count table#_________________________________________________________________
#Read count tables
...
#read data
Sol_ann <- read.table(paste0(path, "01_Data/SoldatovData/", 
                             "emb_joint.txt"), 
...
# Prep sce-objects
## Remove genes that are not in both data sets#_________________________________
...
## Make Cao sce-object#_________________________________________________________
...
## Make Soldatov sce-object#____________________________________________________
...
## Construct scmap cluster index#_______________________________________________
...
## Project Soldatov on Cao#_____________________________________________________
...
#Export results
saveRDS(list(Sol_sce, Sol_scClus_res), 
        paste0(path, "02_Analysis/07_scmapSolOnCao_output_", 
               Sys.Date(), ".Rds"))
#_______________________________________________________________________________
print("step 11")

#Plot the results#______________________________________________________________
...
ggsave(plot = p2, filename = paste0(path,"03_Results/",
                                    "07_scmapSolOnCao_simp_",
                                    Sys.Date(), ".pdf"))
#_______________________________________________________________________________
print("step 13")

#### Count cells________________________________________________________________
#How many were classified as NCC?
length(Sol_scClus_res_renamed$scmap_cluster_labs[
  Sol_scClus_res_renamed$scmap_cluster_labs == "NCC"])
length(Sol_scClus_res_renamed$scmap_cluster_labs[
  Sol_scClus_res_renamed$scmap_cluster_labs == "Not NCC"])
length(Sol_scClus_res_renamed$scmap_cluster_labs[
  Sol_scClus_res_renamed$scmap_cluster_labs == "Possible NCC"])
length(Sol_scClus_res_renamed$scmap_cluster_labs[
  Sol_scClus_res_renamed$scmap_cluster_labs == "unassigned"])
#_______________________________________________________________________________
print("step 14")
