...
#Load packages
suppressMessages(library("tidyverse", lib.loc ="/usr/local/lib/R/site-library"))
library("emmeans", lib.loc = "/usr/local/lib/R/site-library")
library("latex2exp", lib.loc = "/usr/local/lib/R/site-library")

#Set working directory
path <- "~/WorkDirs/PmurNCCPilot_10X_rerun/"

#Read and setup data from the two qPCR-plates
Pool5 <- read.csv(paste0(path, "01_Data/qPCR_Pool5_AutoBT49.4.csv"))
...
Pool5$Well[20:nrow(Pool5)] <- paste("Plate1_", Pool5$Well[20:nrow(Pool5)], 
                                    sep = "")

Pool6 <- read.csv(paste0(path, "01_Data/qPCR_Pool6_AutoBT81.95.csv"))
...
Pool6$Well[20:nrow(Pool6)] <- paste("Plate2_", Pool6$Well[20:nrow(Pool6)], 
                                    sep = "")
...
Normalized target gene Cq is calculates as: $a = 2^{(b−c)}$ \
Where \
